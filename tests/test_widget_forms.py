...
class TestTextboxBehavior():
    ...
    def test_widget_forms_textbox_text_dispatches_update(self, txt):
        txt.text = "hello"
        assert len(txt.view.received_commands) == 1 and txt.view.received_commands[0] == {
            'name': 'text', 'selector': '#' + txt.id, 'text': "hello"}, "Setting TextBox.text should dispatch an event if a view is attached."
